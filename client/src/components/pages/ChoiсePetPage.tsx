...
export default function ChoiсePetPage(): JSX.Element {
  ...
  const pets = useAppSelector((state) => state.user);

  useEffect(() => {
    dispatch(getAllMatchThunk());
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        <Button sx={{ backgroundColor: '#F3EDED', borderRadius: '10px' }} variant="outlined" onClick={navigateToInfo}>
          Добавить питомца
        </Button>
      </Box>
      <Box
